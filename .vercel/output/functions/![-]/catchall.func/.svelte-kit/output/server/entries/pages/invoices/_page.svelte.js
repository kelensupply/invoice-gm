...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let filteredInvoices = derived(() => store_get($$store_subs ??= {}, "$invoices", invoices).filter((inv) => {
      return true;
    }));
    let tableData = derived(() => filteredInvoices().map((inv) => ({
      raw: inv,
    ...
    head("dmf30d", $$renderer2, ($$renderer3) => {
      $$renderer3.title(($$renderer4) => {
        $$renderer4.push(`<title>Invoices - Invoicer App</title>`);
      });
    });
    PageHeader($$renderer2, {
      ...
      children: ($$renderer3) => {
        AppButton($$renderer3, {
          ...
          size: "sm",
          children: ($$renderer4) => {
            $$renderer4.push(`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 mr-1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"></path></svg> New Invoice`);
          },
          $$slots: { default: true }
        });
      }
    });
    $$renderer2.push(`<!----> <div class="mb-5 overflow-x-auto"><div class="border-b border-slate-200"><nav class="-mb-px flex space-x-1 sm:space-x-2 min-w-max" aria-label="Invoice filter tabs"><!--[-->`);
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      if (tab.value === "all") {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<span class="ml-1.5 bg-slate-100 text-slate-600 py-0.5 px-2 rounded-full text-xs font-medium">${escape_html(store_get($$store_subs ??= {}, "$invoices", invoices).length)}</span>`);
      } else {
        $$renderer2.push("<!--[-1-->");
        const count = store_get($$store_subs ??= {}, "$invoices", invoices).filter((i) => tab.value === "unpaid" ? i.status !== "paid" && i.status !== "overdue" : i.status === tab.value).length;
        if (count > 0) {
          $$renderer2.push("<!--[0-->");
    ...
    $$renderer2.push(`<!--]--></nav></div></div> `);
    {
      let emptyStateAction = function($$renderer3) {
        AppButton($$renderer3, {
          ...
          class: "mt-4",
          children: ($$renderer4) => {
            $$renderer4.push(`<!---->Create your first invoice`);
          },
          $$slots: { default: true }
        });
      }, row = function($$renderer3, rowData) {
        $$renderer3.push(`<td class="px-5 py-4 whitespace-nowrap text-sm font-bold text-slate-900">${escape_html(rowData.invoiceNumber)}</td> <td class="px-5 py-4 whitespace-nowrap text-sm font-medium text-slate-700">${escape_html(rowData.client)}</td> <td class="px-5 py-4 whitespace-nowrap">`);
